...
possible to define a relabelling function that :
...
arm haveing missing values, but 25% of the treament arm: 
...
options("tableone.show_pvalue_method"=TRUE)

# This comparison implicitly ignores missing values.
discrete_cgd %>% compare_population(formula)

# N.B. The following option is important when converting integer data:
...
cautious of the findings. If factor values are missing (as in this case) then we
can include them as a new group and get a more robust comparison which includes
the distribution of missingness. In contrast to the previous comparison
...
Beyond the bio-medical example `tableone` can make any more general comparison between
data that has a structure like:

`~ group + observation_1 + observation_2 + ... + observation_n`

We will use the `iris` and the `diamonds` datasets to demonstrate this more general
use case for `tableone`. 
