...
* A custom labeller function defined for the table
* Control of whether or not to display the footer.
...
# we can choose to hide the footer
...
* Show the method for the p-value as daggers
...
old = options(
  ...
  "tableone.show_pvalue_method"=TRUE
)
...
the column contents. The column contents can refer to the following variables:

* `subtype_count` can use `{level}`, `{prob.0.5}`, `{prob.0.025}`, `{prob.0.975}`, `{n}`, `{N}` - `n` is subgroup count, `N` is data count.
...
* `.sprintf_na` - sprintfs a set of numbers replacing the output with 
  `getOption("tableone.na","\u2014")` if all values are missing, and if some
  values are missing relacing each individial missing value with  
  `getOption("tableone.missing","<?>")`
* `.sprintf_no_na`  - sprintfs a set of numbers replacing the output with 
...
# old = options("tableone.hide_footer"=TRUE)
# or by setting `footer_text` to false
